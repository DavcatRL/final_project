...
class Iiwa_action : public rclcpp::Node
{
    ...
    private:
        ...
        void joint_state_subscriber(const sensor_msgs::msg::JointState& sensor_msg){
            ...
            // Mappa dei giunti dell'arm
            std::unordered_map<std::string,int> joint_map = {
                {"joint_a1",0},{"joint_a2",1},{"joint_a3",2},
                {"joint_a4",3},{"joint_a5",4},{"joint_a6",5},{"joint_a7",6}
            };

            // Aggiorna i valori dei joint dell'arm basandosi sul nome
            for(size_t i=0; i<sensor_msg.name.size(); i++){
                auto it = joint_map.find(sensor_msg.name[i]);
                if(it != joint_map.end()){
                    joint_positions_(it->second) = sensor_msg.position[i];
                    joint_velocities_(it->second) = sensor_msg.velocity[i];
                }
            }
        }
