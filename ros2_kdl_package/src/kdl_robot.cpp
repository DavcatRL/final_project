...
void KDLRobot::createChain(KDL::Tree &robot_tree)
{
    ...
    std::cout << "DEBUG: chain_.getNrOfJoints() = " << chain_.getNrOfJoints() << std::endl;
    
    // Stampa TUTTI i giunti nella chain
    std::cout << "Chain joints:" << std::endl;
    for(unsigned int i=0; i<chain_.getNrOfSegments(); i++) {
        KDL::Segment seg = chain_.getSegment(i);
        KDL::Joint jnt = seg.getJoint();
        if(jnt.getType() != KDL::Joint::None) {
            std::cout << "  " << i << ": " << jnt.getName() 
                      << " (" << jnt.getTypeName() << ")" << std::endl;
        }
    }
...
// ADD FAKE GRIPPER FUNCTION
void KDLRobot::addFakeGripper() {
    // Parametri della pinza/fake link
    double mass = 0.1;  // massa totale della pinza (kg)
    
    // Centro di massa relativo al tool0
    KDL::Vector com(0, 0, -0.00025); // in metri
    
    // Inerzia rispetto al centro di massa
    double Ixx = 2.88e-4;
    double Iyy = 1.71e-4;
    double Izz = 1.74e-4;

    // Crea inertia del link
    KDL::RigidBodyInertia rbi(mass, com, KDL::RotationalInertia(Ixx, Iyy, Izz));

    KDL::Frame link7_tool0_offset(KDL::Vector(0.0, 0.0, 0.154));

    // Segmento con fixed joint
    KDL::Segment fakeSegment("fake_gripper",
                            KDL::Joint(KDL::Joint::None),   // fixed joint
                            KDL::Frame::Identity(),         // posizione relativa a tool0
                            rbi);

    // Aggiungi segmento alla chain
    chain_.addSegment(fakeSegment);

    std::cout << "[INFO] Added fake end-effector 'fake_gripper' to KDL chain" << std::endl;
}

void KDLRobot::updateChainSolvers() {
    // Aggiorna il numero di giunti (non cambia se il link è fixed)
    n_ = chain_.getNrOfJoints();

    // Ridimensiona gli array dei giunti
    jntArray_.resize(n_);
    jntVel_.resize(n_);
    grav_.resize(n_);
    coriol_.resize(n_);
    ...
    std::cout << "[INFO] checkpoint1" << std::endl;

    // Distruggi eventuali solver esistenti
    if(dynParam_)     delete dynParam_;
    if(fkSol_)        delete fkSol_;
    if(fkVelSol_)     delete fkVelSol_;
    if(jacSol_)       delete jacSol_;
    if(jntJacDotSol_) delete jntJacDotSol_;
    if(idSolver_)     delete idSolver_;
    if(ikVelSol_)     delete ikVelSol_;
    if(ikSol_)        delete ikSol_;

    std::cout << "[INFO] checkpoint2" << std::endl;

    // Ricrea solver sulla nuova chain
    dynParam_     = new KDL::ChainDynParam(chain_, KDL::Vector(0,0,-9.81));
    fkSol_        = new KDL::ChainFkSolverPos_recursive(chain_);
    fkVelSol_     = new KDL::ChainFkSolverVel_recursive(chain_);
    jacSol_       = new KDL::ChainJntToJacSolver(chain_);
    ...
    std::cout << "[INFO] KDL solvers updated after adding link" 
              << ". Chain joints: " << n_ 
              << ", segments: " << chain_.getNrOfSegments() << std::endl;
}
